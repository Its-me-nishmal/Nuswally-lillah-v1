...
class NotificationService {
  ...
  static Future<void> init() async {
    ...
    try {
      ...
      debugPrint(
        'NotificationService: Local timezone initialized to $timeZoneName',
      );
    } catch (e) {
      debugPrint(
        'NotificationService: Failed to get local timezone, falling back to Asia/Kolkata: $e',
      );
    ...
    try {
      await _notificationsPlugin.initialize(
        ...
        onDidReceiveNotificationResponse: (NotificationResponse response) {
          ...
          debugPrint(
            'Notification Action Triggered: $actionId for payload: $payload',
          );
    ...
    if (androidImplementation != null) {
      try {
        ...
        debugPrint(
          'NotificationService: Failed to create adhan_full_channel: $e',
        );
      }
      ...
      try {
        ...
        debugPrint(
          'NotificationService: Failed to create adhan_chime_channel: $e',
        );
      }
      ...
      try {
        ...
        await androidImplementation.createNotificationChannel(
          const AndroidNotificationChannel(
            ...
            description:
                'Silent reminders shown 10 minutes before prayer alerts',
        ...
        debugPrint(
          'NotificationService: Failed to create silent_upcoming_channel: $e',
        );
      }
      ...
      try {
        ...
        debugPrint(
          'NotificationService: Failed to create adhan_default_channel: $e',
        );
      }
      ...
      try {
        ...
        debugPrint(
          'NotificationService: Failed to create app_update_channel: $e',
        );
      }
      ...
      try {
        ...
        await androidImplementation.createNotificationChannel(
          const AndroidNotificationChannel(
            ...
            description:
                'Live ongoing prayer times and countdown pinned to lock screen',
            importance: Importance.low,
            playSound: false,
        ...
        debugPrint(
          'NotificationService: Failed to create sticky_prayer_channel: $e',
        );
      }
    }
  }
  ...
  static Future<void> requestPermissions() async {
    ...
    if (androidImplementation != null) {
      ...
      try {
        ...
        debugPrint(
          'NotificationService: Error requesting notifications permission: $e',
        );
      }

      // Request exact alarm permission (required for Android 13/14+)
      try {
        final bool? isExactAlarmPermitted = await androidImplementation
            .canScheduleExactNotifications();
        if (isExactAlarmPermitted == false) {
          debugPrint(
            'NotificationService: Exact alarms not permitted. Prompting user...',
          );
          await androidImplementation.requestExactAlarmsPermission();
        } else {
          debugPrint(
            'NotificationService: Exact alarms permission is already granted.',
          );
        }
      } catch (e) {
        debugPrint(
          'NotificationService: Error checking or requesting exact alarm permission: $e',
        );
      }
    }
  }
  ...
  static Future<void> schedulePrayerNotifications({
    ...
    int idCounter =
        10000; // Offset for scheduled IDs to avoid overlaps with immediate ones
    ...
    try {
      ...
      debugPrint(
        'NotificationService: Error querying exact alarm permission: $e',
      );
    }
    ...
    debugPrint(
      'NotificationService: Scheduling Azan alarms (Pre-Azan delay: $preAzanReminderMinutes min, Mode: ${scheduleMode.name})...',
    );
      ...
      for (var prayerName in prayers) {
        ...
        try {
          ...
          final triggerTime = preAzanReminderMinutes > 0
              ? prayerDateTime.subtract(
                  Duration(minutes: preAzanReminderMinutes),
                )
              : prayerDateTime;
          ...
          if (triggerTime.isAfter(tzNow)) {
            ...
            const AndroidNotificationDetails androidDetails =
                AndroidNotificationDetails(
                  ...
                  'Adhan Alarms',
                  channelDescription:
                      'Alarms sounding for daily Azan prayer times',
                  importance: Importance.max,
          ...
          debugPrint(
            'Error scheduling Azan alarm for $prayerName on $dateStr: $e',
          );
        }
      }
    }
  ...
  static Future<void> silenceAllAlarmsAndAzan() async {
    try {
      await _notificationsPlugin.cancelAll();
      debugPrint(
        'NotificationService: All notification alarms and Azan sound silenced.',
      );
    } catch (e) {
  ...
  static Future<void> showUpdateNotification(AppUpdateInfo info) async {
    ...
    await _notificationsPlugin.show(
      ...
      body:
          'New version v${info.versionName} is ready with fresh features and improvements. Tap to update!',
      notificationDetails: platformDetails,
      payload: 'app_update',
    );
  }
  ...
  static Future<void> checkAndNotifyUpdates() async {
    try {
      debugPrint(
        'NotificationService: Checking for daily app updates in background...',
      );
      final updateInfo = await AppUpdateService.fetchUpdateInfo(
        forceRemote: true,
      );
      if (updateInfo != null &&
          AppUpdateService.isUpdateAvailable(updateInfo)) {
        debugPrint(
          'NotificationService: Update found (v${updateInfo.versionName}). Triggering notification.',
        );
  ...
  static Future<void> updateStickyPrayerNotification({
    ...
    try {
      ...
      final androidDetails = AndroidNotificationDetails(
        ...
        color: const Color(0xFF14B8A6), // brand accent (no BuildContext here),
      ...
      debugPrint(
        'NotificationService: updateStickyPrayerNotification error: $e',
      );
    }
  }
  ...
  static Future<void> cancelStickyPrayerNotification() async {
    try {
      ...
      debugPrint(
        'NotificationService: cancelStickyPrayerNotification error: $e',
      );
    }
  }
}
